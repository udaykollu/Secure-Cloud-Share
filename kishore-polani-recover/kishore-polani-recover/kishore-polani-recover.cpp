...

{
	...
	std::ifstream myfile3(efile);

	if (myfile3.is_open())
	{
		while (getline(myfile3, line))
		{
			if ((line.compare("HH") == 0))
			{
				//cout << "tag start"  << endl;
				cstatus = false;
			}
...
//	cout << "hmac: " << encodedd << endl;
	...
	{
		cout << "File data integrity check failed exiting without decryption";
		system("pause");
		exit(1);
	}
	/*********************************\
...
//	std::cout << std::endl << std::endl;
	...
	std::ofstream myfile0("sfile.txt");
	if (myfile0.is_open())
	{

		myfile0 << decryptedtext << endl;
		myfile0.close();
	}
	
	cout << "Plain text is recovered to sfile.txt";
